import { Mesh, Program, Renderer, Triangle } from 'https://esm.sh/ogl@1.0.11?target=es2022';
...
const config = {
  color: '#19b7cb', secondaryColor: '#A78BFA', trailLength: 40, trailWidth: 8,
  trailTaper: 0.8, followSpeed: 0.16, glowIntensity: 1.9, glowSpread: 1.2,
  hotspot: 0.65, brightness: 1.25, opacity: 0.56, pulseSpeed: 1.1,
  noiseStrength: 0.035, idleFade: true, idleTimeout: 700, fadeDuration: 900,
  blendMode: 'screen', maxDevicePixelRatio: 1.5, enabled: true
};
...
const container = document.querySelector('.site-glow-cursor');
const canvas = container?.querySelector('.glow-cursor__canvas');

if (container && canvas && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
  canvas.style.mixBlendMode = config.blendMode;
  const renderer = new Renderer({ canvas, alpha: true, dpr: Math.min(devicePixelRatio || 1, config.maxDevicePixelRatio) });
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uHotspot: { value: config.hotspot }, uBrightness: { value: config.brightness }, uOpacity: { value: config.opacity },
      uPulseSpeed: { value: config.pulseSpeed }, uNoiseStrength: { value: config.noiseStrength },
      uNormalBlend: { value: config.blendMode === 'normal' ? 1 : 0 }, uTime: { value: 0 }, uFade: { value: 0 }
    }, transparent: true, depthTest: false, depthWrite: false
  });
  const mesh = new Mesh(gl, { geometry: new Triangle(gl), program });
  let width = 1, height = 1, initialized = false, pointerInside = false, fade = 0;
  let lastInputTime = performance.now(), lastFrameTime = performance.now(), raf = 0;
  ...
  const initializeTrail = (x, y) => {
    Object.assign(target, { x, y }); Object.assign(head, { x, y });
  ...
  const updatePointer = event => {
    ...
    Object.assign(target, { x, y }); pointerInside = true; lastInputTime = performance.now();
  };
  const render = now => {
    const delta = Math.min((now - lastFrameTime) / 16.667, 3); lastFrameTime = now;
    if (initialized) {
  ...
  const onPointerLeave = () => { pointerInside = false; lastInputTime = performance.now(); };
